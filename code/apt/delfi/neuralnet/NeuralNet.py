...
from delfi.utils.symbolic import MyLogSumExp
...
class NeuralNet(object):
    def __init__(self, n_inputs, n_outputs, n_components=1, n_filters=[],
                 n_hiddens=[10, 10], n_rnn=None, n_inputs_rnn=1,
                 impute_missing=True, seed=None, svi=True):
        ...
        n_rnn : None or int
            Number of RNN units
        n_inputs_rnn: None or int
            Dimensionality of input to RNN at each time step. Used only when
            len(n_inputs) == 1.
        impute_missing : bool
        ...
        if self.n_rnn > 0:
            if len(self.n_inputs) == 1:
                assert np.mod(np.prod(n_inputs), n_inputs_rnn) == 0
                rs = (-1, self.n_inputs[0] // n_inputs_rnn, n_inputs_rnn)
