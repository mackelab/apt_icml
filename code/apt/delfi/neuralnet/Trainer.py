...
class ActiveTrainer(Trainer):
    ...
    def __init__(self, network, loss, trn_data, trn_inputs,
                 ...
                 generator=None, n_null=1, moo='resample', obs=None):
        """Construct and configure the trainer
        ...
        n_null: int
            Number of additional simulation parameters to draw for discrete-
            proposal SNPE-C loss
        moo: string
    ...
    def assemble_extra_inputs_mdn(self, trn_data, generator, n_null,
            moo='resample', obs=None):
        """convenience function for assembling input for network training"""

        batchsize = trn_data[0].shape[0]
        if moo == 'resample':
            # reuse other theta_n from the same batch as theta_null

            th_nl = np.empty((batchsize, n_null, trn_data[0].shape[1]), dtype=dtype)
            for n in range(batchsize):
                idx = self.rng.choice(batchsize-1, n_null, replace=False) + n + 1
                th_nl[n, :, :] = trn_data[0][np.mod(idx, batchsize)]
            theta_all = np.concatenate(
                (trn_data[0].reshape((batchsize, 1, -1)), th_nl), axis=1)
        else:
            raise NotImplemented('mode of operation not supported')

        # compute log prior ratios (assuming uniform discrete proposal)
        lprs = generator.prior.eval(
            theta_all.reshape(batchsize * (n_null + 1), -1),
            log=True).reshape(batchsize, n_null + 1).astype(dtype)
    ...
    def assemble_extra_inputs_maf(self, trn_data, generator, n_null,
            moo='resample', obs=None):
        """convenience function for assembling input for network training.
        ...
        if moo == 'resample':
            ...
            if n_null < batchsize - 1:
                # reuse other theta_n from the same batch as theta_null

                idx_ = np.empty((batchsize, n_null), dtype=int)
                for n in range(batchsize):
                    idx_[n] = self.rng.choice(batchsize - 1, n_null,
                                              replace=False)
        ...
        # compute log prior ratios (assuming uniform discrete proposal)
        lprs = generator.prior.eval(th_nl, log=True).reshape(n_null+1, batchsize).astype(dtype)

        # inputs to MAF (summary stats)
        x_nl = np.tile(trn_data[1], (n_null+1, 1))

        # th_nl : (n_batch * (n_null + 1)  x n_outputs
        # lprs  : (n_null+1) x n_batch
        # x_nl  : (n_batch * (n_null+1)) x n_inputs
        return (th_nl, x_nl, lprs, *trn_data[2:])

    def sample_cond_maf(self, n_samples, n_null, x):

        mades, rng = self.network.mades, self.rng
        y = rng.randn(n_samples*n_null, self.network.n_outputs).astype(dtype)
        if getattr(self.network, 'batch_norm', False):

            for made, bn in zip(mades[::-1], self.network.bns[::-1]):
                y = bn.eval_inv(y)
            ...
            for made in mades[::-1]:
                #y = made.gen(x, n_samples, y, rng)
                u = y.copy()
                y = np.zeros([n_samples*n_null, made.n_outputs], dtype=dtype)
                xy = (np.tile(x, (n_null,1)), y)

                for i in range(1, self.network.n_outputs + 1):
                    m, logp = made.eval_comps(xy)
                    idx = np.argwhere(made.output_order == i)[0, 0]
