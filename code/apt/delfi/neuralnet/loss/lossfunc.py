...
from delfi.utils.symbolic import (tensorN, tensorQF, mog_LL, MyLogSumExp,
                                  invert_each, det_each)

def snpeabc_loss_prior_as_proposal(model, svi=False):
    """
    Simplest loss function, for the case where the prior is the same as the
    proposal. In this case snpe-A, snpe-B and snpe-C are equivalent, and we're
    simply maxmimizing the likelihood of params under the parameterized mdn.
    ...
    if isinstance(model, NeuralNet):
        params = tensorN(2, name='params', dtype=dtype)  # (batch, n_outputs)
        stats = model.stats
        # a is mixture weights, ms are means, U^T U are precisions
        a, ms, Us, ldetUs = model.get_mog_tensors(svi=svi)
        loss = -tt.mean(
            mog_LL(params, tt.log(a), ms, Us, ldetUs, from_cholesky=True))
    else:
        ...
        params = model.y     # (batch, n_outputs)
        stats = model.input  # (batch, n_inputs)
        loss = -tt.mean(model.L)  # negative average log-likelihood

    trn_inputs = [params, stats]
    return loss, trn_inputs


def snpec_loss_MoG_proposal(mdn, prior, n_proposal_components=None, svi=False):
    ...
    trn_inputs : list
        Tensors to be provided to the loss function during training
    """
    assert isinstance(mdn, NeuralNet)
...
def snpec_loss_gaussian_proposal(mdn, prior, svi=False):
    ...
    loss = -tt.mean(mog_LL(params, pp_las, pp_ms, pp_Ps, pp_ldetPs))

    # collect extra input variables to be provided for each training data point
    trn_inputs = [params, mdn.stats, prop_m, prop_P]

    return loss, trn_inputs


def snpec_loss_discrete_proposal(model, svi=False, combined_loss=False):
    """Define loss function for training with a discrete proposal. Assumes a
    ...
    """
    if isinstance(model, NeuralNet):
        return snpec_mdn_loss_discrete_proposal(model, svi=svi,
            combined_loss=combined_loss)
    else:
        from snl.ml.models.mafs import ConditionalMaskedAutoregressiveFlow
        assert isinstance(model, ConditionalMaskedAutoregressiveFlow)
        assert not svi, 'SVI not supported for MAFs'
        return snpec_maf_loss_discrete_proposal(model, svi=svi,
            combined_loss=combined_loss)


def snpec_mdn_loss_discrete_proposal(mdn, svi=False, combined_loss=False):
    """Define loss function for training with a discrete proposal. Assumes a
    uniform proposal distribution over each sample parameter and an externally
    provided set of alternatives.
    ...
    # theta_all : (n_batch * (n_null + 1)  x n_outputs
    # lprs  : n_batch x (n_null+1)
    theta_all = tensorN(3, name='params_nl', dtype=dtype)  # true (row 1), nulls
    ...
    # diffs : [ n_batch x (n_null+1) x n_outputs for each component ]
    # Ms    : [ n_batch x (n_null+1)             for each component ]
...
def snpec_maf_loss_discrete_proposal(maf, svi=False, combined_loss=False):
    """Define loss function for training with a discrete proposal. Assumes a
    uniform proposal distribution over each sample parameter and an externally
    provided set of alternatives.
    ...
    svi : bool
        Whether to use SVI version of the mdn or not
    """
    #assert isinstance(maf, ConditionalMaskedAutoregressiveFlow)

    assert not svi, 'SVI not supported for MAFs'

    # define symbolic variable to hold params that will be inferred
    # params : n_batch x  n_outputs
    # all_thetas : (n_batch * (n_null + 1)  x n_outputs
    # lprs  : (n_null+1) x n_batch
    # stats :  n_batch x  n_inputs
    # x_nl  : (n_batch * (n_null+1)) x n_inputs
    params, stats = maf.y, maf.input
    theta_all = tensorN(2, name='params_nl', dtype=dtype)
    x_nl = tensorN(2, name='stats_nl', dtype=dtype)
    lprs = tensorN(2, name='lprs', dtype=dtype) # log tilde_p / p

    n_batch = tt.shape(lprs)[1]
    n_null = tt.shape(lprs)[0] - 1

    # compute MAF log-densities for each true and null theta
    lprobs = theano.clone(output=maf.L, replace={params:theta_all, stats:x_nl},
        share_inputs=True)
    lprobs = tt.reshape(lprobs, newshape=(n_null+1, n_batch), ndim=2)

    # compute nonnormalized log posterior probabilities for the discrete task
    discrete_ppZ = lprobs - lprs
    # compute posterior probability of true params in discrete task
    discrete_pp = discrete_ppZ[0, :].squeeze() - \
        MyLogSumExp(discrete_ppZ, axis=0).squeeze()
    ...
    if combined_loss:  # add prior loss on prior samples
        ...
        is_prior_sample = tensorN(1, name='prop_mask', dtype=dtype)
        trn_inputs.append(is_prior_sample)
        loss = -tt.mean(discrete_pp + is_prior_sample * l_ml)
    else:
        loss = -tt.mean(discrete_pp)
